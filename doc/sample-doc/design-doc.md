...
| 画面名 | 概要 | 主な機能 |
|-------|-----|---------|
| **ログイン画面**    | ユーザー認証を行う | ユーザー名とパスワードの入力、ログイン処理 |
| **ダッシュボード画面** | システム全体の概要や重要な情報を表示する | ステータス表示、アクティビティログ |
| **設定画面**        | NW機器の設定を行う画面| 設定項目の一覧表示、設定値の入力、設定内容の保存機能。 |
| **モニタリング画面** | NW機器の状態をリアルタイムで監視する | ステータス表示、グラフやチャートによる視覚化 |
